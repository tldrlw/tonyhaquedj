...
function log(level, message, extra = {}) {
  ...
  const rec = { severity: (level || "info").toUpperCase(), message, ...extra };
  try {
    if (typeof console[method] === "function") console[method](safe(rec));
    else console.log(safe(rec));
  } catch {
    console.log(safe({ severity: rec.severity, message }));
  }
  try {
...
async function resolveBytes({ bucket, name, sizeFromEvent }) {
  ...
  if (!FORCE_METADATA_SIZE) {
    ...
    log("info", "resolveBytes:forceMetadataPath");
  }
...
/* ---------------- BigQuery helpers ---------------- */
let _tableRef = null; // memoize across warm invocations
async function getTable() {
  if (_tableRef) return _tableRef;
...
async function processObject({
  ...
  const bytes = Number.isFinite(sizeBytes) && sizeBytes >= 0 ? sizeBytes : null;
  const sizeMB =
